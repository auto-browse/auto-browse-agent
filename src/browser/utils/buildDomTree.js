...
const args = {
  ...
  function highlightElement(element, index, parentIframe = null) {
    ...
    try {
      ...
      for (const rect of rects) {
        ...
        overlay.style.border = '2px solid ' + baseColor; // Change 1
        ...
        overlay.style.top = top + 'px'; // Change 2
        overlay.style.left = left + 'px'; // Change 3
        overlay.style.width = rect.width + 'px'; // Change 4
        overlay.style.height = rect.height + 'px'; // Change 5
      ...
      label.style.fontSize = Math.min(12, Math.max(8, firstRect.height / 2)) + 'px'; // Change 6
      ...
      label.style.top = labelTop + 'px'; // Change 7
      label.style.left = labelLeft + 'px'; // Change 8
      ...
      const updatePositions = () => {
        ...
        overlays.forEach((overlayData, i) => {
          if (i < newRects.length) { // Check if rect still exists
            ...
            overlayData.element.style.top = newTop + 'px'; // Change 9
            overlayData.element.style.left = newLeft + 'px'; // Change 10
            overlayData.element.style.width = newRect.width + 'px'; // Change 11
            overlayData.element.style.height = newRect.height + 'px'; // Change 12
        ...
        if (label && newRects.length > 0) {
          ...
          label.style.top = newLabelTop + 'px'; // Change 13
          label.style.left = newLabelLeft + 'px'; // Change 14
  ...
  function isInteractiveElement(element) {
    ...
    try {
      ...
      // console.warn('Could not check event listeners for element:', e);
      // If checking listeners fails, rely on other checks
    }
  ...
  function handleHighlighting(nodeData, node, parentIframe, isParentHighlighted) {
    ...
    if (!isParentHighlighted) {
      ...
      if (isElementDistinctInteraction(node)) {
        ...
        // console.log('Skipping highlight for node (parent highlighted)');
    ...
    if (shouldHighlight) {
      ...
      if (nodeData.isInViewport) {
        ...
        // console.log('Skipping highlight for node (outside viewport)');
      }
    }
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false) {
    ...
    if (node === document.body) {
      ...
      const id = ID.current++; // Removed template literal
  ...
  if (debugMode && PERF_METRICS) {
    ...
    Object.keys(PERF_METRICS.buildDomTreeBreakdown.domOperations).forEach(op => {
      ...
      if (count > 0) {
        const keyName = op + 'Average';
        PERF_METRICS.buildDomTreeBreakdown.domOperations[keyName] = time / count;
      }
    });
